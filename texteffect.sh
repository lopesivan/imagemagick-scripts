#!/usr/bin/env bash
#
# Developed by Fred Weinhaus 3/31/2008 .......... revised 11/28/2011
...
trap "rm -f $tmpA $tmpB $tmpA $tmpB $tmpA0 $tmpB0 $tmpA1 $tmpB1 text_label.txt; exit 0" 0
...
if [ "$style" = "plain" ]
	...
	convert -size $dim xc:"$bc" \
		...
		$fill -strokewidth $linewt -stroke $oc \
		-gravity center -annotate 0x${italic}-${blur1}-${blur1} "$text" \
		$trimpad +repage $tmpA0
...
if [ "$effect" = "normal" ]
	then
	convert $tmpA0 -background "$bc" -trim -bordercolor "$bc" -border $pad \
		$tmpA0
	...
	then
	if [ "$effect" = "wedge-top" -o "$effect" = "wedge-bottom" ]; then
		if [ "$im_version" -ge "06050102" ]; then
	...
	convert $tmpA0 $vp -background $bc \
		-distort bilinearforward "$coords" \
	...
	if [ "$im_version" -ge "06040307" ]
		...
		if [ "$effect" = "bulge-top" -o "$effect" = "bulge-bottom" -o "$effect" = "pinch-top" -o "$effect" = "pinch-bottom" ]
		  then
		  convert $tmpA0 $vp -trim +repage $tmpA0
		fi
